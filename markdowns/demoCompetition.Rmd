...
require(tidyr)
chart <- fpds %>%
    select(Contracting.Group.ID, compCat, catAward, Action.Obligation) %>%
    mutate(Competition = compCat) %>%
    group_by(Contracting.Group.ID, Competition, catAward) %>%
    summarize(dollars = sum(Action.Obligation)*10^-6 , actions = n()) %>%
    gather(.,"measure","value", 4:5)

chart$Competition <- factor(chart$Competition, 
                        levels = c("Effectively Competed","One Bid","Not Competed"))

ggplot(data = filter(chart, !is.na(Competition))
