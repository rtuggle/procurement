createFrame <- function(type, file) {
    ...
    for(i in 2:len) {
        doc <- children[[i]]
        #set type
        if(tolower(type) == 'award') {
            x <- 'awardID'
            y <- 'awardContractID'
        } else if(toupper(type) == 'IDV') {
            x <- 'contractID'
            y <- 'IDVID'
        } else {
            stop('Please enter a valid type. Should be either award or IDV.') 
        }
        ##PIID
        var <- xmlApply(doc[[x]][[y]], xmlValue)
        ...
        var <- try(xmlApply(doc[[x]][['referencedIDVID']], xmlValue), silent = TRUE)
        ...
        ##DATE
        var <- xmlApply(doc[['relevantContractDates']], xmlValue)
        ...
        ##DOLLAR
        var <- xmlApply(doc[['dollarValues']], xmlValue)
        ...
        ##PURCHASER
        var <- xmlApply(doc[['purchaserInformation']], xmlValue)
        ...
        ##Marketing
        var <- xmlApply(doc[['contractMarketingData']], xmlValue)
        ...
        ##CONTRACT
        var <- xmlApply(doc[['contractData']], xmlValue)
        ...
        ##PRODUCT 
        var <- xmlApply(doc[['productOrServiceInformation']], xmlValue)
        ...
        ##COMPETITION 
        var <- xmlApply(doc[['competition']], xmlValue)
        ...
        ##VENDOR
        var <- xmlApply(doc[['vendor']][['vendorHeader']], xmlValue)
        ...
        ###Socio
        var <- xmlApply(doc[['vendor']][['vendorSiteDetails']][['vendorSocioEconomicIndicators']], xmlValue)
        ...
        ###Minority
        var <- xmlApply(doc[['vendor']][['vendorSiteDetails']][['vendorSocioEconomicIndicators']][['minorityOwned']], xmlValue)
        ...
        ###Certifications
        var <- xmlApply(doc[['vendor']][['vendorSiteDetails']][['vendorCertifications']], xmlValue)
        ...
        ###Location
        var <- xmlApply(doc[['vendor']][['vendorSiteDetails']][['vendorLocation']], xmlValue)
        ...
        ###SiteCode
        var <- xmlApply(doc[['vendor']][['vendorSiteDetails']], xmlValue)
        ...
        ###DUNS
        var <- xmlApply(doc[['vendor']][['vendorSiteDetails']][['vendorDUNSInformation']], xmlValue)
